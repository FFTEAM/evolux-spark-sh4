#!/bin/sh
CURDIR=`pwd`
NRELDIR='../../tufsbox/release_neutrino-mp'
CHANGEDIR='../../tufsbox'
...
#	;;
#esac
$USERS chmod 777 -R $CHANGEDIR/release_neutrino-mp/dev
if [ -e $CHANGEDIR/release_neutrino-mp_with_dev ]; then
	$USERS rm -rf $CHANGEDIR/release_neutrino-mp_with_dev
fi

mv $CHANGEDIR/release_neutrino-mp $CHANGEDIR/release_neutrino-mp_with_dev
if [ ! -e $BUILDDIR/.spark7162 ]; then
	if [ -e $BUILDDIR/own_build/neutrino/boot/audio.elf ] || [ -e $BUILDDIR/own_build/neutrino/boot/video.elf ]; then
		cp -RP $BUILDDIR/own_build/neutrino/boot/*.elf $CHANGEDIR/release_neutrino-mp_with_dev/boot/
	fi
	...
	if [ -e $BUILDDIR/own_build/neutrino/boot/spark7162/audio.elf ] || [ -e $BUILDDIR/own_build/neutrino/boot/spark7162/video.elf ]; then
		cp -RP $BUILDDIR/own_build/neutrino/boot/spark7162/*.elf $CHANGEDIR/release_neutrino-mp_with_dev/boot/
	fi
else
	echo "#############################"
	echo "#############################"
...
if [ -e $PATCHDIR/custom/myPatches_Neutrino-MP.diff ]; then
	cd $CHANGEDIR/release_with_dev && patch -p1 < "../../custom/myPatches_Neutrino-MP.diff"
fi
	#$USERS chown root:root $CHANGEDIR/release_neutrino-mp_with_dev/bin/lcd4linux
	#$USERS chmod 700 $CHANGEDIR/release_neutrino-mp_with_dev/etc/lcd4linux.conf
#	cp -RP $CHANGEDIR/release_neutrino-mp_with_dev/boot/uImage $CHANGEDIR/
#	$CHANGEDIR/host/bin/mkfs.jffs2 -r $CHANGEDIR/release_neutrino-mp_with_dev -o $CHANGEDIR/e2jffs2.img -e 0x20000 -n
#	( cd $CHANGEDIR && tar -czvf neutrino-mp_on_Pingulux.tar.gz e2jffs2.img uImage )
echo "--- Erledigt ---"
exit
