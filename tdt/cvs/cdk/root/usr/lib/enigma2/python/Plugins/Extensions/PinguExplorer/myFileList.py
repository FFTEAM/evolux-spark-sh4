...
def FileEntryComponent(name, absolute = None, isDir = False):
	...
	if isDir:
		png = LoadPixmap("/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/dir.png")
	else:
		extension = name.split('.')
		extension = extension[-1].lower()
		if EXTENSIONS.has_key(extension):
			png = LoadPixmap("/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/" + EXTENSIONS[extension] + ".png")
		else:
			png = None
	if png is not None:
		...
		if self.showFiles:
			for x in files:
				if self.useServiceRef:
					...
					name = x
					nx = None
					if (config.plugins.PinguExplorer.MediaFilter.value == "on"):
				...
				if (self.matchingPattern is None) or (EXext in self.matchingPattern):
					if nx is None:
						...
						png = LoadPixmap("/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/movie.png")
						res.append((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHATEST, 12, 3, 20, 20, png))
