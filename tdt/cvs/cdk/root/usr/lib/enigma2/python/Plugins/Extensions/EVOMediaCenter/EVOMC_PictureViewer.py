from enigma import ePicLoad, eTimer, getDesktop, evfd
...
from GlobalFunctions import EVOMC_FolderOptions, EVOMC_FavoriteFolders, EVOMC_FavoriteFolderAdd, EVOMC_FavoriteFolderEdit

#import os
from os import path, system

config.plugins.EVOMC_pp = ConfigSubsection()
config.plugins.EVOMC_pp.slidetime = ConfigInteger(default=10, limits=(5, 60))
config.plugins.EVOMC_pp.resize = ConfigSelection(default="0", choices = [("0", _("simple")), ("1", _("better"))])
config.plugins.EVOMC_pp.cache = ConfigEnableDisable(default=True)
config.plugins.EVOMC_pp.lastDir = ConfigText(default=resolveFilename(SCOPE_MEDIA))
config.plugins.EVOMC_pp.rotate = ConfigSelection(default="0", choices = [("0", _("none")), ("1", _("manual")), ("2", _("by Exif"))])
config.plugins.EVOMC_pp.ThumbWidth = ConfigInteger(default=145, limits=(1, 999))
config.plugins.EVOMC_pp.ThumbHeight = ConfigInteger(default=120, limits=(1, 999))
config.plugins.EVOMC_pp.bgcolor = ConfigSelection(default="#00000000", choices = [("#00000000", _("black")),("#009eb9ff", _("blue")),("#00ff5a51", _("red")), ("#00ffe875", _("yellow")), ("#0038FF48", _("green"))])
config.plugins.EVOMC_pp.textcolor = ConfigSelection(default="#0038FF48", choices = [("#00000000", _("black")),("#009eb9ff", _("blue")),("#00ff5a51", _("red")), ("#00ffe875", _("yellow")), ("#0038FF48", _("green"))])
config.plugins.EVOMC_pp.framesize = ConfigInteger(default=30, limits=(5, 99))
config.plugins.EVOMC_pp.infoline = ConfigEnableDisable(default=True)
config.plugins.EVOMC_pp.loop = ConfigEnableDisable(default=True)
...
class EVOMC_PicThumbViewer(Screen, HelpableScreen):
	...
	def __init__(self, session, piclist, lastindex, path):
		...
		self.textcolor = config.plugins.EVOMC_pp.textcolor.value
		self.color = config.plugins.EVOMC_pp.bgcolor.value
		textsize = 20
		self.spaceX = 20
		self.spaceY = 28
		self.picX = config.plugins.EVOMC_pp.ThumbWidth.value
		self.picY = config.plugins.EVOMC_pp.ThumbHeight.value
		...
		for x in range(self.thumbsC):
			...
			skincontent += "<widget name=\"label" + str(x) + "\" position=\"" + str(absX+5) + "," + str(absY+self.picY-textsize) + "\" size=\"" + str(self.picX - 10) + ","  + str(textsize) + "\" font=\"Regular;16\" zPosition=\"2\" transparent=\"1\" noWrap=\"1\" foregroundColor=\"" + self.textcolor + "\" />"
		...
		self.skin = "<screen position=\"0,0\" size=\"" + str(size_w) + "," + str(size_h) + "\" flags=\"wfNoBorder\" > \
			<ePixmap pixmap=\"skin_default/buttons/green.png\" position=\"225,530\" zPosition=\"2\" size=\"140,40\" transparent=\"1\" alphatest=\"on\" /> \
			<ePixmap pixmap=\"skin_default/buttons/yellow.png\" position=\"390,530\" zPosition=\"2\" size=\"140,40\" transparent=\"1\" alphatest=\"on\" /> \
			<ePixmap pixmap=\"skin_default/buttons/blue.png\" position=\"555,530\" zPosition=\"2\" size=\"140,40\" transparent=\"1\" alphatest=\"on\" /> \
			<widget name=\"key_green\" position=\"205,526\" zPosition=\"3\" size=\"140,40\" backgroundColor=\"#1f771f\" font=\"Regular;18\" valign=\"center\" halign=\"center\" transparent=\"1\" /> \
			<widget name=\"key_yellow\" position=\"373,526\" zPosition=\"3\" size=\"140,40\" backgroundColor=\"#1f771f\" font=\"Regular;18\" valign=\"center\" halign=\"center\" transparent=\"1\" /> \
			<widget name=\"key_blue\" position=\"538,526\" zPosition=\"3\" size=\"140,40\" backgroundColor=\"#1f771f\" font=\"Regular;18\" valign=\"center\" halign=\"center\" transparent=\"1\" /> \
			<eLabel position=\"0,0\" zPosition=\"0\" size=\""+ str(size_w) + "," + str(size_h) + "\" backgroundColor=\"" + self.color + "\" /> \
			<widget name=\"frame\" position=\"35,30\" size=\"" + str(self.picX +1) + "," + str(self.picY +10) + "\" pixmap=\"/usr/lib/enigma2/python/Plugins/Extensions/EVOMediaCenter/skins/" + str(self.skindir) + "/images/icons/pic_frame.png\" zPosition=\"3\" alphatest=\"on\" />"  + skincontent + "</screen>"
	...
	def setPicloadConf(self):
		sc = getScale()
		self.picload.setPara([self["thumb0"].instance.size().width(), self["thumb0"].instance.size().height(), sc[0], sc[1], config.plugins.EVOMC_pp.cache.value, int(config.plugins.EVOMC_pp.resize.value), self.color])
		self.paintFrame()
	...
	def KeyOk(self):
		...
		self.session.openWithCallback(self.callbackView, EVOMC_PicView, self.filelist, self.index, self.path, False)

	def startslideshow(self):
		if self.maxentry < 0:
			return
		
		self.session.openWithCallback(self.callbackView, EVOMC_PicView, self.filelist, self.index, self.path, True)
...
class EVOMC_PicView(Screen):
	def __init__(self, session, filelist, index, path, startslide):

		self.textcolor = config.plugins.EVOMC_pp.textcolor.value
		self.bgcolor = config.plugins.EVOMC_pp.bgcolor.value
		space = config.plugins.EVOMC_pp.framesize.value
		size_w = getDesktop(0).size().width()
		size_h = getDesktop(0).size().height()
		...
		self.skin = "<screen position=\"0,0\" size=\"" + str(size_w) + "," + str(size_h) + "\" flags=\"wfNoBorder\" > \
			...
			<widget name=\"point\" position=\""+ str(space+5) + "," + str(space+2) + "\" size=\"20,20\" zPosition=\"2\" pixmap=\"/usr/lib/enigma2/python/Plugins/Extensions/EVOMediaCenter/skins/" + str(self.skindir) + "/images/icons/record.png\" alphatest=\"on\" /> \
	...
	def setPicloadConf(self):
		sc = getScale()
		self.picload.setPara([self["pic"].instance.size().width(), self["pic"].instance.size().height(), sc[0], sc[1], 0, int(config.plugins.EVOMC_pp.resize.value), self.bgcolor])
		
		self["play_icon"].hide()
		if config.plugins.EVOMC_pp.infoline.value == False:
			self["file"].hide()
		self.start_decode()
	...
	def slidePic(self):
		...
		if config.plugins.EVOMC_pp.loop.value==False and self.lastindex == self.maxentry:
			self.PlayPause()
		self.shownow = True
		self.ShowPicture()
	...
	def PlayPause(self):
		if self.slideTimer.isActive():
			...
			self.slideTimer.start(config.plugins.EVOMC_pp.slidetime.value*1000)
			self["play_icon"].show()
			self.nextPic()
...
class EVOMC_PicSetup(Screen):
	def __init__(self, session):
		...
		self.list.append(getConfigListEntry(_("Slideshow Interval (sec.)"), config.plugins.EVOMC_pp.slidetime))
		self.list.append(getConfigListEntry(_("Scaling Mode"), config.plugins.EVOMC_pp.resize))
		self.list.append(getConfigListEntry(_("Cache Thumbnails"), config.plugins.EVOMC_pp.cache))
		self.list.append(getConfigListEntry(_("Thumbnail Width"), config.plugins.EVOMC_pp.ThumbWidth))
		self.list.append(getConfigListEntry(_("Thumbnail Height"), config.plugins.EVOMC_pp.ThumbHeight))
		self.list.append(getConfigListEntry(_("show Infoline"), config.plugins.EVOMC_pp.infoline))
		self.list.append(getConfigListEntry(_("Frame size in full view"), config.plugins.EVOMC_pp.framesize))
		self.list.append(getConfigListEntry(_("slide picture in loop"), config.plugins.EVOMC_pp.loop))
		self.list.append(getConfigListEntry(_("backgroundcolor"), config.plugins.EVOMC_pp.bgcolor))
		self.list.append(getConfigListEntry(_("textcolor"), config.plugins.EVOMC_pp.textcolor))
