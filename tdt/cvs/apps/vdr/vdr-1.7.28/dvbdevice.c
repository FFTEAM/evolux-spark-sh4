...
#define SETCMD(c, d) { Frontend[CmdSeq.num].cmd = (c);\
  ...
  if (ioctl(fd_frontend, FE_SET_PROPERTY, &CmdSeq) < 0) {
     esyslog("ERROR: set frontend %d/%d: %m", adapter, frontend);
     return false;
     }
  CmdSeq.num = 0;
...
#if !defined (PLATFORM_STB71XX) && !defined (PLATFORM_SMP86XX)
/* todo for sh */
  // Common Interface:
...
bool cDvbDevice::QueryDeliverySystems(int fd_frontend)
{
  ...
  if (Result == 0) {
     ...
     dsyslog("frontend fd %d type %d\n", fd_frontend, frontendInfo.type);
     ...
     dsyslog("frontend type %d on frontend %d/%d", frontendInfo.type, adapter, frontend);
     }
