...
#ifdef USE_REELCHANNELSCAN // reelchannelscan
...
int cDevice::set_filter(int fd, int pid) {
 ...
 esyslog("SOFTCAM_SWITCH: Error in setting section filter");
 return -1;
 }
 return 0;
}
...
int cDevice::get_pmt_pid(unsigned char *buffer, int SID) {
 ...
 esyslog("PMT pid: '0x%X (%d)'", pmt_pid, pmt_pid);
 return pmt_pid;
 }
 }
 return 0;
...
void cDevice::WritePmtTmp(char DvbNum, int pmtid) {
 ...
 char *demux_dev;
 char *fnam="tmp/pmt.tmp";
 char *fnax;
 asprintf(&demux_dev, "/dev/dvb/adapter0/demux%d",DvbNum);
 asprintf(&fnax, "/tmp/pmt%d.tmp",DvbNum);
 esyslog(demux_dev);
 if ((fd = open(demux_dev, O_RDWR)) < 0) {
 esyslog("SOFTCAM_SWITCH: Error opening demux device");
 } else {
 if (set_filter(fd, 0) < 0) {
 esyslog("SOFTCAM_SWITCH: Error in set filter pat");
 }
 if ((length = read_t(fd, buffer)) < 0) {
 esyslog("SOFTCAM_SWITCH: Error in read read_t (pat)");
 } else {
 pmt_pid=this->get_pmt_pid(buffer, pmtid);
 if (pmt_pid==0) {
 esyslog("SOFTCAM_SWITCH: Error pmt_pid not found");
 } else {
 if (this->set_filter_pmt(fd, pmt_pid) < 0) {
 esyslog("SOFTCAM_SWITCH: Error in set pmt filter");
 }
 for (k=0; k<64; k++) {
 if ((length = this->read_t(fd, buffer)) < 0) {
 esyslog("SOFTCAM_SWITCH: Error in read pmt\n");
 }
...
// SOFTCAM
esyslog("SOFTCAM_SWITCH Device: '%d' Channel: '%d' SID '%d' ", this->DeviceNumber(), Channel->Number(), Channel->Sid());
