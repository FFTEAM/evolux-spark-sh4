...
const CMenuOptionChooser::keyval VIDEOMENU_VIDEOMODE_OPTIONS[VIDEOMENU_VIDEOMODE_OPTION_COUNT] =
{
	...
	{ 10, NONEXISTANT_LOCALE, "1080p 50Hz"	},
				...
				CMenuWidget &service, CMenuWidget &fontSettings, CMenuWidget &audiopl_picSettings, CMenuWidget &streamingSettings, CMenuWidget &moviePlayer, CMenuWidget &ExtraMenu) 
{
	...
	mainMenu.addItem(new CMenuForwarder(LOCALE_INETRADIO_NAME, true, NULL, audioPlayer, NULL, CRCInput::convertDigitToKey(shortcut++)));
...
//*****************************************************************************************************************
	...
	mainMenu.addItem( new CMenuForwarder(LOCALE_DBOXINFO, true, NULL, new CDBoxInfoWidget, NULL, CRCInput::RC_info /* CRCInput::RC_help */, "help"));
	...
	mainSettings.addItem(new CMenuForwarder(LOCALE_MAINSETTINGS_LANGUAGE  , true, NULL, &languageSettings , NULL, CRCInput::RC_6));

	xmlDocPtr parser;

	parser = parseXmlFile("/etc/timezone.xml");
	if (parser != NULL) {
		tzSelect = new CMenuOptionStringChooser(LOCALE_MAINSETTINGS_TIMEZONE, g_settings.timezone, true, new CTZChangeNotifier(), CRCInput::RC_7, "", true);
...
void CNeutrinoApp::InitServiceSettings(CMenuWidget &service, CMenuWidget &scanSettings)
{
	...
	zapit_menu->addItem(new CMenuOptionChooser(LOCALE_EXTRA_ZAPIT_MAKE_NEWBOUQUET, (int *)&zapitCfg.makeNewChannelsBouquet, OPTIONS_OFF0_ON1_OPTIONS, OPTIONS_OFF0_ON1_OPTION_COUNT, true));
	...
	CDataResetNotifier * resetNotifier = new CDataResetNotifier();
	service.addItem(new CMenuForwarder(LOCALE_RESET_CHANNELS    , true, NULL, resetNotifier, "channels"));
	service.addItem(new CMenuForwarder(LOCALE_SERVICEMENU_GETPLUGINS, true, NULL, this, "reloadplugins"));

	service.addItem(GenericMenuSeparatorLine);
	service.addItem(new CMenuForwarder(LOCALE_SERVICEMENU_IMAGEINFO,  true, NULL, new CImageInfo(), NULL, CRCInput::RC_info /* CRCInput::RC_help */, NEUTRINO_ICON_BUTTON_HELP_SMALL ), false);
	...
	if(0 /*softupdate*/)
	{
		...
		const char * versionString = (configfile.loadConfig("/var/etc/.version")) ? (configfile.getString( "version", "????????????????????").c_str()) : "????????????????????";
...
#ifdef DUCKBOX
#define MISCSETTINGS_SCALE_DISPLAY_TYPE_OPTION_COUNT 3
const CMenuOptionChooser::keyval MISCSETTINGS_SCALE_DISPLAY_TYPE_OPTIONS[MISCSETTINGS_SCALE_DISPLAY_TYPE_OPTION_COUNT] =
{
	...
	{ 2, LOCALE_SCALE_DISPLAY_TYPE_RAINBOW }
};
#endif
...
#define CPU_FREQ_OPTION_COUNT 5
...
void CNeutrinoApp::InitMiscSettings(CMenuWidget &miscSettings)
{
	...
	miscSettings.addItem(new CMenuOptionNumberChooser(LOCALE_FAN_SPEED, &g_settings.fan_speed, true, 1, 5, funNotifier, 0, 0, LOCALE_OPTIONS_OFF) );
...
#define AUDIOMENU_VOLUME_POSITION_OPTION_COUNT 3
const CMenuOptionChooser::keyval AUDIOMENU_VOLUME_POSITION_OPTIONS[AUDIOMENU_VOLUME_POSITION_OPTION_COUNT] =
{
        { 0, LOCALE_AUDIOMENU_VOLUME_POSITION_UPPERLEFT },
        { 1, LOCALE_AUDIOMENU_VOLUME_POSITION_CENTER  },
        { 2, LOCALE_AUDIOMENU_VOLUME_POSITION_HIDDEN  },
};
...
void CNeutrinoApp::InitAudioSettings(CMenuWidget &audioSettings, CAudioSetupNotifier* audioSetupNotifier)
{
	...
	audioSettings.addItem(new CMenuOptionChooser(LOCALE_AUDIOMENU_AC3DOWNMIX, &g_settings.audio_ac3downmix, OPTIONS_OFF0_ON1_OPTIONS, OPTIONS_OFF0_ON1_OPTION_COUNT, true, audioSetupNotifier));
	...
	audioSettings.addItem(new CMenuOptionChooser(LOCALE_AUDIOMENU_VOLUME_POSITION, &g_settings.volslider_pos, AUDIOMENU_VOLUME_POSITION_OPTIONS, AUDIOMENU_VOLUME_POSITION_OPTION_COUNT, true, NULL));
#if 0
...
CMenuWidget_Network::CMenuWidget_Network(const neutrino_locale_t Name, const std::string & Icon, const int mwidth, const int mheight)
 : CMenuWidget(Name, Icon, mwidth, mheight)
	{
	networkSettings_Interface = NULL;
}

void CMenuWidget_Network::InitInterfaceList()
{
	if (networkSettings_Interface)
		networkSettings_Interface->clearOptions();
	...
	if (!getifaddrs(&ifap)) {
		...
		it = ifap;
		while (it) {
			if (it->ifa_name && strcmp(it->ifa_name, "lo"))
				networkSettings_Interface->addOption(it->ifa_name);
			it = it->ifa_next;
		}
		freeifaddrs(ifap);
	}
...
void WLAN_Menu::WLAN_Settings() {
	...
	CStringInputSMS *WlanEssidInput = new CStringInputSMS(LOCALE_NETWORKMENU_WLAN_ESSID,
		tmp_essid, sizeof(tmp_essid) - 1, NONEXISTANT_LOCALE, NONEXISTANT_LOCALE,
		"ABCDEFGHIJKLMNOPQRSTUVWXYZ@abcdefghijklmnopqrstuvwxyz0123456789!�$%&/[]()=?-:\\ '#*+<>_");
	CMenuForwarder* m0 = new CMenuForwarder(LOCALE_NETWORKMENU_WLAN_ESSID, true, tmp_essid, WlanEssidInput);

	CStringInputSMS *WlanKeyInput = new CStringInputSMS(LOCALE_NETWORKMENU_WLAN_KEY, tmp_key,
		sizeof(tmp_key) - 1, NONEXISTANT_LOCALE, NONEXISTANT_LOCALE,
		"ABCDEFGHIJKLMNOPQRSTUVWXYZ@abcdefghijklmnopqrstuvwxyz0123456789!�$%&/[]()=?-:\\ '#*+<>_");
	CMenuForwarder* m1 = new CMenuForwarder(LOCALE_NETWORKMENU_WLAN_KEY, true, tmp_key, WlanKeyInput);

	CMenuOptionChooser* m2 = new CMenuOptionChooser(LOCALE_NETWORKMENU_WLAN_MODE, &wlan_mode,
	...
	menu->addItem(m2);
	menu->addItem(GenericMenuSeparatorLine);
	menu->addItem(new CMenuForwarder(LOCALE_NETWORKMENU_WLAN_RESET, true, "", this, "reset",
		CRCInput::RC_red, NEUTRINO_ICON_BUTTON_RED));
	menu->addItem(new CMenuForwarder(LOCALE_NETWORKMENU_WLAN_APPLY, true, "", this, "apply",
		CRCInput::RC_green, NEUTRINO_ICON_BUTTON_GREEN));
	menu->exec(NULL, "");

	menu->hide();

	delete WlanEssidInput;
	delete WlanKeyInput;
	delete menu;
}

void CNeutrinoApp::InitNetworkSettings(CMenuWidget_Network &networkSettings)
{
...
//	CMenuForwarder *m3 = new CMenuForwarder(LOCALE_NETWORKMENU_BROADCAST , networkConfig.inet_static, networkConfig.broadcast , networkSettings_Broadcast );
	...
	dhcpNotifier = new CDHCPNotifier(m1,m2,/*m3,*/m4,m5);

/*	network_automatic_start = networkConfig.automatic_start ? 0 : 1;
	CMenuOptionChooser* oj = new CMenuOptionChooser(LOCALE_NETWORKMENU_SETUPONSTARTUP, &network_automatic_start, OPTIONS_OFF0_ON1_OPTIONS, OPTIONS_OFF0_ON1_OPTION_COUNT, true, new CNetworkAutostartChangeNotifier);
*/
	network_automatic_start = 1;
	...
	networkSettings.addItem(new CMenuForwarder(LOCALE_NETWORKMENU_SHOW, true, NULL, this, "networkshow", CRCInput::RC_info /* CRCInput::RC_help */ , NEUTRINO_ICON_BUTTON_HELP_SMALL));
	networkSettings.addItem( m0 );

	networkSettings.addItem(GenericMenuSeparatorLine);

	networkSettings.InitInterfaceList();
	if(networkSettings.networkSettings_Interface) {
		networkSettings.addItem(networkSettings.networkSettings_Interface);
		networkSettings.addItem(networkSettings.wlanMenuForw);
		networkSettings.wlanMenuForw->setActive(CNeutrinoApp::getInstance()->networkConfig.isWireless());
	}

	networkSettings.addItem(GenericMenuSeparatorLine);
	network_dhcp = networkConfig.inet_static ? 0 : 1;
	DHCPOptionChooser = new CMenuOptionChooser(LOCALE_NETWORKMENU_DHCP, &network_dhcp, OPTIONS_OFF0_ON1_OPTIONS, OPTIONS_OFF0_ON1_OPTION_COUNT, true, dhcpNotifier);
	networkSettings.addItem(DHCPOptionChooser);
	networkSettings.addItem(GenericMenuSeparatorLine);
...
//	networkSettings.addItem(GenericMenuSeparatorLine); // broadcast, will be calculated from ip and mask
...
const char *CNeutrinoApp::getUserMenuButtonName(int button)
{
        if(button < 0 || button >= SNeutrinoSettings::BUTTON_MAX)
                return false;
	...
	#define locCheck(L) if(loc != NONEXISTANT_LOCALE) return_title = true; else loc=L;continue
        ...
        for(int pos = 0; pos < SNeutrinoSettings::ITEM_MAX && !return_title; pos++) {
				...
				if (g_RemoteControl->current_PIDs.APIDs.size() > 0)
                        		text = g_RemoteControl->current_PIDs.APIDs[g_RemoteControl->current_PIDs.PIDs.selected_apid].desc;
				locCheck(LOCALE_AUDIOSELECTMENUE_HEAD);
			case SNeutrinoSettings::ITEM_SUBCHANNEL:
				if (!g_RemoteControl->subChannels.empty()) {
					locCheck(LOCALE_VIDEOMENU_HEAD);
				}
				return_title = true;
				continue;
                        case SNeutrinoSettings::ITEM_TECHINFO:
