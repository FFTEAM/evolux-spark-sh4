...
void freesatHuffmanDecoder::loadTables()
{
	if (! m_tablesLoaded )
	{
		...
		if (!loadFile(1, TABLE1_FILENAME))
			loadFile(1, TABLE1_FILENAME_BAK);
		if (!loadFile(2, TABLE2_FILENAME))
			loadFile(2, TABLE2_FILENAME_BAK);
		m_tablesLoaded = true;
	}
}
...
*/
bool freesatHuffmanDecoder::loadFile(int tableid, const char *filename)
{
	char     buf[1024];
	char    *from, *to, *binary;
	FILE    *fp;
	...
	{
		fprintf(stderr, "[FREESAT] Cannot load <%s> for table %d\n",filename,tableid + 1);
		return false;
	}
}
...
std::string freesatHuffmanDecoder::decode(const unsigned char *src, size_t size)
{
	...
	if (src[0] == 0x1f && (src[1] == 1 || src[1] == 2))
	{
		...
		do
		{
			...
			{
				fprintf(stderr, "[FREESAT] Missing table %d entry: <%s>\n", tableid + 1, uncompressed.c_str());
				return uncompressed;
			}
