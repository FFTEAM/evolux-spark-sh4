#######################################################################
#
#    Pingu-ExplorerII for Pingulux-Enigma2
...
config.plugins.PinguExplorer = ConfigSubsection()
config.plugins.PinguExplorer.startDir = ConfigText(default="/")
config.plugins.PinguExplorer.MediaFilter = ConfigText(default="off")
config.plugins.PinguExplorer.CopyDest = ConfigText(default="/")
...
def Plugins(**kwargs):
	list = [PluginDescriptor(name="Pingulux-Explorer", description=_("Explore your Pingulux."), where = [PluginDescriptor.WHERE_PLUGINMENU], icon="pinguexplorer.png", fnc=main)]
 	list.append(PluginDescriptor(name=_("Pingulux-Explorer"), where = PluginDescriptor.WHERE_EXTENSIONSMENU, fnc=main))
...
def showExpl(dummy_self = None):
	global explSession
	explSession.open(PinguExplorerII)



######## DREAM-EXPLORER START #######################
class PinguExplorerII(Screen):
	global HDSkn
	if HDSkn:
		if (getDesktop(0).size().width()) > 1030:
			skin = """
				<screen position="center,80" size="1180,590" title="Pingu-Explorer">
				<widget name="filelist" position="5,10" scrollbarMode="showOnDemand" size="962,572" zPosition="4"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/red.png" position="995,20" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/green.png" position="995,60" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/yellow.png" position="995,100" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/blue.png" position="995,140" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/menu.png" position="995,180" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/info.png" position="995,220" size="35,25" zPosition="5"/>
			...
			skin = """
				<screen position="center,77" size="900,450" title="Pingu-Explorer">
				...
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/red.png" position="0,425" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/green.png" position="155,425" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/yellow.png" position="310,425" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/blue.png" position="465,425" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/menu.png" position="620,425" size="35,25" zPosition="5"/>
				<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/info.png" position="775,425" size="35,25" zPosition="5"/>
		...
		skin = """
			<screen position="center,77" size="620,450" title="Pingu-Explorer">
			...
			<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/red.png" position="0,425" size="35,25" zPosition="5"/>
			<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/green.png" position="145,425" size="35,25" zPosition="5"/>
			<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/yellow.png" position="290,425" size="35,25" zPosition="5"/>
			<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/blue.png" position="430,425" size="35,25" zPosition="5"/>
			<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/info.png" position="555,425" size="35,25" zPosition="5"/>
			<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/res/menu.png" position="585,425" size="35,25" zPosition="5"/>
	...
	def __init__(self, session, args = None):
		self.skin = PinguExplorerII.skin
		...
		self.booklines = []
		self.MediaPattern = "^.*\.(ts|m2ts|mp3|wav|ogg|jpg|jpeg|jpe|png|bmp|mpg|mpeg|mkv|mp4|mov|divx|avi|mp2|m4a|flac|ifo|vob|iso|sh|flv|3gp|mod)"
		if pathExists(config.plugins.PinguExplorer.startDir.value):
			StartMeOn = config.plugins.PinguExplorer.startDir.value
		else:
			StartMeOn = None
		if (config.plugins.PinguExplorer.MediaFilter.value == "off"):
		...
		if self["filelist"].canDescent():
			...
			if filename != None:
				if testFileName.endswith(".ts"):
					...
					dei.setTitle(_("Pingu-Explorer : Install..."))
					...
					self.commando = [ "/usr/lib/enigma2/python/Plugins/Extensions/PinguExplorer/pyc2xml " + filename ]
					...
					dei.setTitle(_("Pingu-Explorer : Disassemble..."))
	...
	def updateLocationInfo(self):
		try:
			...
			self.setTitle(_("Pingu-Explorer"))
	...
	def SysExecution(self, answer):
		...
		if answer == "YES":
			...
			if self.MediaFilter:
				self.MediaFilter=False
				config.plugins.PinguExplorer.MediaFilter.value = "off"
				config.plugins.PinguExplorer.MediaFilter.save()
				self["filelist"].matchingPattern = None
				self["filelist"].refresh()
				self.updateLocationInfo()
			else:
				self.MediaFilter=True
				config.plugins.PinguExplorer.MediaFilter.value = "on"
				config.plugins.PinguExplorer.MediaFilter.save()
	...
	def Info(self):
		if self["filelist"].canDescent():
			if self["filelist"].getSelectionIndex()!=0:
				...
				dei = self.session.open(MessageBox, _("Pingubox: " + self.MyBox + "\n\n" + ScanSysem_str()), MessageBox.TYPE_INFO)
				dei.setTitle(_("Pingu-Explorer"))
		else:
	...
	def ExecDelete(self):
		...
		if not(self["filelist"].canDescent()):
			...
			dei.setTitle(_("Pingu-Explorer - DELETE file..."))
			...
			dei.setTitle(_("Pingu-Explorer - DELETE DIRECTORY..."))
	...
	def callbackSetStartDir(self, answerSD):
		if answerSD is True:
			config.plugins.PinguExplorer.startDir.value = self["filelist"].getSelection()[0]
			config.plugins.PinguExplorer.startDir.save()
...
class CPmaniger(Screen):
	...
	def __init__(self, session, source = "/tmp/none"):
		...
		self["CPto"] = myFileList(config.plugins.PinguExplorer.CopyDest.value, showDirectories = True, showFiles = False, matchingPattern = "^.*\.*", useServiceRef = False)
	...
	def CopyFile(self):
		if self["CPto"].getSelectionIndex()!=0:
			...
			try:
				config.plugins.PinguExplorer.CopyDest.value = dest
				config.plugins.PinguExplorer.CopyDest.save()
				os_system(order)
			except:
				dei = self.session.open(MessageBox,_("%s \nFAILED!" % order), MessageBox.TYPE_ERROR)
				dei.setTitle(_("Pingu-Explorer"))
...
class SoftLinkScreen(Screen):
	...
	def GetSLname(self):
		self.session.openWithCallback(self.callbackSetLinkName, vInputBox, title=_("Write the new softlink name here:"), windowTitle=_("Pingu Explorer..."), text="newname")
